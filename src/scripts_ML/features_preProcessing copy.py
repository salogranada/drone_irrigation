...
flight_df = pd.read_csv('../data_base/flight_data/flight_data_2000_parte3.txt', sep="|")
...
path_df = pd.read_csv('../data_base/random_paths/path_v4_header_p3_salo.txt', sep="|")

#Opens data frame for error file.
error_df = pd.read_csv('../data_base/reports/error_report_2000paths_parte3.txt', sep="|")

#Creates new data frame for saving energies
energy_df = path_df
energy_df['Energy'] = 0 #Adds energy column
...
#Removes from energy data frame the paths that reported error in simulation. 
for j, row_log in error_df.iterrows():
    current_route = row_log.loc['route']
    log = row_log.loc['log']

    indexs = energy_df.index
    condition = energy_df["path_num"] == current_route
    error_indices = indexs[condition]

    if log.startswith('DRONE'):
        energy_df.drop([error_indices])
    elif log.startswith('Negative'):
        energy_df.drop([error_indices])
    elif log.startswith('TARGET'):
        energy_df.drop([error_indices])

# apples_indices_list = error_indices.tolist()
