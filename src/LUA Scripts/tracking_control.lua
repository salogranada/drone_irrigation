...
function sysCall_init() 
    ...
    pRoll, iRoll, dRoll = 9,0,0.005
    pPitch, iPitch, dPitch = 128,1,0.005
...
function sysCall_actuation() 
    ...
    if desired_roll > 0.55 then
        desired_roll = 0.55
    elseif desired_roll < -0.55 then
        desired_roll = -0.55
    end
    if desired_pitch > 0.55 then
        desired_pitch = 0.55
    elseif desired_pitch < -0.55 then
        desired_pitch = -0.55
    end
    ...
    thrust = (masa + tankMass) * (pv + iParam*cumul + dParam*(e-lastE) + l[3]*vParam)
    lastE=e
