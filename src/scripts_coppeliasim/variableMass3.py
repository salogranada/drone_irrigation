...
class variableMass(object):
    ...
    def callback_simulationTime(self,data):
        self.simulationTime = data.data
    def callback_activation(self,data):
        self.irrigationFlag = data.data

    def main(self):
        rospy.init_node('variableMass', anonymous=True)
        rospy.Subscriber('/irrigation_flag', String, self.callback_activation)
        rospy.Subscriber('/simulationTime',Float32,self.callback_simulationTime)
        pub = rospy.Publisher('currentMass', Float32,queue_size=50)
        pub_init_flag = rospy.Publisher('init_flag', Bool,queue_size=50)

        print('Waiting for command...')
        while not rospy.is_shutdown():
            #print(self.irrigationFlag)
            if self.irrigationFlag != 'initial':
                print('Entro al calculo')
                mass_calculation(self)
                actual_time = (self.simulationTime - self.simTimeIrrigation0)/1000 #in seconds
                idx = close_time_value(self.matrixValues[:,2],actual_time)
                print(idx)
                pub.publish(self.matrixValues[idx,1])
                pub_init_flag.publish(True)
            else:
    ...
    def mass_calculation(self):
        ...
        if self.irrigationFlag == 'B10L': #Begins irrigation (Default 10L).
            ...
            t = np.linspace(0,int(tFinal+5),int(num=tFinal+5)) #linspace with 5 seconds of excess
